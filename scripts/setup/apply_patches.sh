...
apply_patch_file() {
    ...
    if git -C "${target_dir}" apply --check "${patch_file}" >/dev/null 2>&1; then
        if (( DRY_RUN )); then
            echo "[dry-run] apply ${rel_patch} -> ${target_dir}"
        else
            git -C "${target_dir}" apply "${patch_file}"
            echo "Applied ${rel_patch} -> ${target_dir}"
        fi
        return 0
    fi

    if git -C "${target_dir}" apply --reverse --check "${patch_file}" >/dev/null 2>&1; then
        echo "Skipped ${rel_patch}: patch already applied in ${target_dir}"
        return 0
    fi

    echo "无法应用 patch: ${rel_patch} (target: ${target_dir})" >&2
    return 1
}
