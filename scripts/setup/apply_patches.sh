...
usage() {
    cat <<'EOF'
Usage: bash scripts/setup/apply_patches.sh [--method <name>]... [--dry-run]

Apply all patch files under patches/<name>/base/.
...
apply_patch_file() {
    ...
    if git -C "${target_dir}" apply --check "${patch_file}" >/dev/null 2>&1; then
        if (( DRY_RUN )); then
            ...
            echo "[dry-run] apply ${rel_patch} -> ${display_target_dir}"
        else
            git -C "${target_dir}" apply "${patch_file}"
            echo "Applied ${rel_patch} -> ${display_target_dir}"
        fi
        return 0
    fi

    if git -C "${target_dir}" apply --reverse --check "${patch_file}" >/dev/null 2>&1; then
        echo "Skipped ${rel_patch}: patch already applied in ${display_target_dir}"
        return 0
    fi
...
for method in "${METHODS[@]}"; do
    ...
    for patch_file in "${patch_files[@]}"; do
        apply_patch_file "${apply_dir}" "${patch_file}" "${target_dir}"
    done
